...
import { createPost, clearError } from "@/lib/redux/slices/blogSlice";
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function NewPostPage() {
  ...
  useEffect(() => {
    ...
    const suppressCKEditorWarning = (...args: any[]) => {
      ...
      // Suppress CKEditor version security warning/error
    ...
    const suppressCKEditorError = (...args: any[]) => {
      const message = args.join(' ');
      // Suppress CKEditor version security error
      if (message.includes('CKEditor') && (message.includes('not secure') || message.includes('4.18.0'))) {
        return;
      }
    ...
    return () => {
      ...
      // Cleanup: destroy editor instance on unmount
